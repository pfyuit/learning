...
import com.pfyuit.myjavaee.dao.database.jpa.CategoryDao;
...
public class CategoryDaoTest {
	...
	public void testFindByIdByNamedQuery() {
		CategoryModel model = categoryDao.findByIdByNamedQuery(5);
		assertNotNull(model);
		assertNotNull(model.getBlogs());
		BlogDaoTest.printModel(model);
	};
	...
	public void testFindByIdByNativeQuery() {
		CategoryModel model = categoryDao.findByIdByNativeQuery(5);
		assertNotNull(model);
		BlogDaoTest.printModel(model);
	};
	...
	public void testFindAllByNamedQuery() {
		List<CategoryModel> models = categoryDao.findAllByNamedQuery();
		assertNotNull(models);
	};
